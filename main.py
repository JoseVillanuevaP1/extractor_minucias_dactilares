...
def f(input_img):
    ...
    selected_imgs = output_imgs[-4:]

    for i in range(len(selected_imgs)):
        if len(selected_imgs[i].shape) == 2:
            selected_imgs[i] = cv.cvtColor(selected_imgs[i], cv.COLOR_GRAY2RGB)

    # Concatenamos en 2 filas de 2 imágenes
    results = np.concatenate(
        [np.concatenate(selected_imgs[:2], 1), np.concatenate(selected_imgs[2:], 1)]
    ).astype(np.uint8)
